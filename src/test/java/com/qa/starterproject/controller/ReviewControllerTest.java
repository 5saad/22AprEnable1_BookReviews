...
public class ReviewControllerTest {
	...
	void createReviewTest() throws Exception {
		...
		RequestBuilder request = post("/review/create").contentType(MediaType.APPLICATION_JSON)
				.content(this.mapper.writeValueAsString(testReview));
	...
	void readAllReviewTest() throws Exception {
		...
		RequestBuilder request = get("/review/readAll").contentType(MediaType.APPLICATION_JSON)
	...
	void readByIdTest() throws Exception {
		RequestBuilder request = get("/review/read/" + testId).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON);
		ResultMatcher responseStatus = status().isOk();
		ResultMatcher responseContent = content().json(this.mapper.writeValueAsString(testReview));
		this.mock.perform(request).andExpect(responseStatus).andExpect(responseContent);
	...
	void readByRating() throws Exception {
		reviewArray.add(testReview);
		RequestBuilder request = get("/review/rating/" + testRating).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON);
		ResultMatcher responseStatus = status().isOk();
		ResultMatcher responseContent = content().json(this.mapper.writeValueAsString(reviewArray));
		this.mock.perform(request).andExpect(responseStatus).andExpect(responseContent);
	...
	void updateTest() throws Exception {
		...
		RequestBuilder request = put("/review/update/" + testId).contentType(MediaType.APPLICATION_JSON)
				.content(this.mapper.writeValueAsString(newTestReview));
		ResultMatcher responseStatus = status().isOk();
	...
	void deleteTestNotFound() throws Exception {
		RequestBuilder request = delete("/review/delete/342362");
